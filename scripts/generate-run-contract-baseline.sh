#!/usr/bin/env bash
# Manual immutable baseline producer; output must be reviewed before copying into fixtures.
set -euo pipefail
ROOT=$(cd "$(dirname "$0")/.." && pwd)
CONTRACT_REPO=${1:?Usage: generate-run-contract-baseline.sh LOCAL_CONTRACT_REPO [NEW_OUTPUT_DIRECTORY]}
...
cp "$ROOT/tests/run_contract_gate/seed.py" "$ROOT/tests/run_contract_gate/test_run_db.py" "$ROOT/tests/run_contract_gate/__init__.py" "$ROOT/tests/run_contract_gate/baseline.py" "$ROOT/tests/run_contract_gate/baseline_producer.py" "$WORK/repro/run_contract_gate/"
...
"${D[@]}" create --name "$RUNNER" --label "$LABEL=$RUN_ID" --network none \
    ...
    -e RCG_RUNNER_IMAGE="$("${D[@]}" image inspect "$IMAGE" --format '{{.Id}}')" \
    -e RCG_ARCHITECTURE="$("${D[@]}" image inspect "$IMAGE" --format '{{.Os}}/{{.Architecture}}')" \
    "$IMAGE" python -m pytest -q -p no:cacheprovider --confcutdir=/repro \
    /repro/run_contract_gate/baseline_producer.py --junitxml=/output/junit.xml >> "$OUTPUT/resources.txt"
set +e
