...
class TransformedOutcomeRidgeModel:
    ...
    def __post_init__(self) -> None:
        validate_model_payload(self.to_payload())
    ...
    def from_payload(
        ...
        return cls(
            feature_means=tuple(normalized["feature_means"]),
            feature_scales=tuple(normalized["feature_scales"]),
            coefficients=tuple(normalized["coefficients"]),
...
class UpliftModelMetadata:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            status is UpliftModelLifecycleStatus.ACTIVE or self.serving_eligible
        ) and not self.registry_verified:
            raise ValueError(
                "active uplift metadata requires persistent model registry verification"
            )
    ...
    def to_json(self) -> dict[str, Any]:
        ...
        if self.feature_contract_hash is not None:
            payload["feature_contract_hash"] = self.feature_contract_hash
        if self.outcome_contract_hash is not None:
            payload["outcome_contract_hash"] = self.outcome_contract_hash
        return payload
...
def serving_cate_scores(
    *,
    ...
    if (
        metadata.model_lifecycle_status is not UpliftModelLifecycleStatus.ACTIVE
        or not metadata.serving_eligible
        or not metadata.registry_verified
        or metadata.validation_scope != "loopad_randomized_experiments"
        or metadata.model_version != model.model_version
    ):
        return None
    try:
        scores = model.predict_many(examples)
...
def validate_model_payload(payload: Mapping[str, Any]) -> dict[str, Any]:
    if payload.get("model_version") != "transformed-outcome-ridge.v1":
        raise ValueError("unsupported uplift model version")
    raw_means = payload.get("feature_means")
    raw_scales = payload.get("feature_scales")
    raw_coefficients = payload.get("coefficients")
    if not all(
        isinstance(value, (list, tuple))
        for value in (raw_means, raw_scales, raw_coefficients)
    ):
        raise ValueError("uplift model arrays are required")
    try:
        ...
        raise ValueError("uplift model values must be numeric") from exc
    if not means or len(means) != len(scales) or len(means) != len(coefficients):
        raise ValueError("uplift model array dimensions must match")
    numeric_values = [*means, *scales, *coefficients, intercept]
    if not all(math.isfinite(value) for value in numeric_values):
        raise ValueError("uplift model values must be finite")
    if any(scale <= 0 for scale in scales):
        raise ValueError("uplift model feature scales must be positive")
    return {
