        ...
        *,
        ...
        if self._promotion_run_repository.exists_for_promotion_loop(
            ...
            log.warn(
                ...
                {"promotionId": promotion.promotion_id, "loopCount": request.loop_count},
            )
        ...
        log.info(
            "content_candidates_selected",
            {"selectedSegmentCount": len(selected_content)},
        )
        ...
        log.info(
            "promotion_run_prepared",
            {
                "promotionRun": run,
                "adExperimentCount": len(ad_experiments),
            },
        )
    ...
    def _select_analysis(
        ...
        *,
        ...
        if analysis.status != COMPLETED_STATUS:
            log.warn(
                "promotion_analysis_invalid",
                {"analysisId": analysis.analysis_id, "status": analysis.status},
            )
        ...
        *,
        ...
        if generation.status != COMPLETED_STATUS:
            log.warn(
                ...
                {"generationId": generation.generation_id, "status": generation.status},
            )
        ...
        for segment in target_segments:
            ...
            if segment.analysis_id != analysis.analysis_id:
                log.warn(
                    ...
                    {"segmentId": segment.segment_id, "analysisId": segment.analysis_id},
                )
                raise RunValidationError(
    ...
    def _select_content_for_segments(
        ...
        *,
        ...
        for segment in target_segments:
            ...
            if len(candidates) != 1:
                log.warn(
                    "content_candidate_invalid",
                    {
                        "segmentId": segment.segment_id,
                        "contentCandidateCount": len(candidates),
                    },
                )
                raise RunValidationError(
