...
Execution, experiment units, treatment serving assignments는 하나의 PostgreSQL
트랜잭션으로 저장한다.
...
promotion destination이 있으면 destination_id가 그 집합에 포함됨
```
...
음수 CATE는 제거하거나 0으로 보정하지 않는다. 여러 LoopAd randomized experiment가
축적된 뒤 `ad_experiment_id` 단위 cluster bootstrap 1,000회와 percentile 95% CI로
불확실성을 검증한다. 단일 합성 데이터나 외부 데이터의 CI는 serving 활성화 근거가
아니다.
...
리포트에는 treatment/control 균형, ATE, AUUC, Qini, uplift@top-k가 포함된다.
