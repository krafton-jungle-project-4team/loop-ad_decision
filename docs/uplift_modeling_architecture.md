...
- 실행 가능한 predicate 조합을 제한적으로 탐색하는
...
### 구현했지만 승인 전에는 추천 serving에 사용하지 않는 것
...
- cursor 기반 Uplift dataset, experiment time holdout, signed CATE 모델
- validation policy 판정과 persistent model registry
- one-shot 학습 명령, 명시적 승인 activation API, registry serving guard
...
- 종료 실험을 자동 탐색하고 학습하는 scheduler와 worker
- 운영 Uplift 모델의 자동 활성화

현재 운영 DB에는 승인된 Active 모델이 없으므로 lifecycle은 실질적으로
`collecting_data`, `serving_eligible=false`다. Process에서 임의로 만든 metadata로는
활성화할 수 없고, validation policy를 통과한 registry 모델에 대한 명시적 승인이
필요하다.
...
compiler version, audience contract version, beam policy version의 SHA-256이다. 목적지 배열과
...
PostgreSQL Unit은 `(promotion_run_id, ad_experiment_id, user_id)` keyset cursor로
5,000개씩 읽고, Outcome도 최대 5,000명 단위로 조회한다. 최종 선형 학습 Example만
메모리에 유지한다. Dataset manifest에는 정렬된 experiment와 unit 집합 hash,
feature/outcome 계약, vector version, reference time, 학습 코드와 split policy를
봉인한다. 따라서 PostgreSQL page 크기나 ClickHouse batch 크기는 fingerprint에
영향을 주지 않는다.

Train/Test는 사용자 random split이 아니라 experiment 종료 시각 기반 holdout이다.
같은 experiment의 treatment와 control은 항상 같은 partition에 들어가며, 두
partition 모두 양 arm을 포함하지 못하면 학습 불가로 종료한다.
...
정책은 실험 수, arm별 관측·positive outcome, Qini, AUUC baseline을 실제로
판정한다. 통과한 모델은 `validated`까지만 전환되며 자동으로 Active가 되지 않는다.

학습 완료 모델은 `uplift_model_versions`에 dataset fingerprint와 model payload,
metrics, validation 결과를 함께 저장한다. 같은 dataset/code 조합은 한 번만
등록된다. 다음 내부 API가 승인자를 기록하고 한 트랜잭션에서 기존 Active를
retired, 신규 validated 모델을 active로 전환한다.

```text
POST /internal/decision/v1/uplift/models/{model_version_id}/activate
```

Serving guard는 프로젝트, feature contract, outcome contract가 모두 일치하는
`active + serving_eligible + loopad_randomized_experiments` 모델만 로드한다. Registry
장애나 손상 payload가 있으면 CATE를 사용하지 않고 기존 프로모션 적합도와 행동 기반
예약 가능성 순위를 그대로 유지한다.

## 검증 도구

종료된 finalized randomized 실험을 한 번 학습·평가하고 candidate/validated
registry 결과를 출력한다. 동일 dataset fingerprint 재실행은 기존 모델을 반환한다.

```bash
python -m app.uplift.train \
  --project-id demo_project \
  --reference-time 2026-07-22T00:00:00Z
```

이 명령은 자동 scheduler나 background worker가 아니다.
...
- Treatment/Control 결과로 one-shot Uplift 학습·평가를 실행하고 모델과 검증
  결과를 registry에서 버전 관리할 수 있다.
- 검증 정책을 통과한 모델도 별도 승인 API를 거쳐야만 Active가 된다.
- 자동 학습 scheduler와 worker는 구현하지 않았다.
