...
**선택 고객군으로 run을 생성할 때, 실제 PostgreSQL 계약을 통과하고 재시도·실패 이후에도 올바른 데이터가 남는지 로컬과 PR CI에서 반복 검증하는 도구다. 로컬 Gate와 같은 명령의 PR CI workflow를 구현했다. PR 2의 실제 Actions도 통과했다. PR 3A/3B는 다음 검증 경계의 확정된 계획이다.**
...
[기본 명령](developer-guide.md#2-로컬-기본-실행)은 고정·최신 DDL 각각 17개 DB 시나리오와 30개 제어 검사를 실행하고 JSON/JUnit·소스 증거·정리 결과를 남긴다. 고정 baseline의 실제 API가 만든 행과 provenance도 보존했다. PR 1 #395는 개인 통합에 병합됐다. PR 2 #396의 clean 로컬·CI 성공과 개인 통합 병합은 [E-15](evidence-log.md#e-15-pr-2-ci-성공과-개인-통합-병합)에 기록했다. 실제 동시성·Dashboard 소비 검증은 PR 3A·3B로 계획했으며 아직 실행하지 않았다.
...
| 원리 이해 | [학습 안내](learning-guide.md) | 기존 Gate 1~6단원, PR 3 계획 7~9단원의 예시·질문·해설 |
...
| Gate 테스트·컨테이너 실행기·CI | 로컬 Gate·PR 2 CI 완료; PR 3A/3B 계획 확정 |
| 기준 코드가 생성한 기존 row fixture | 생성·새 DB 복원·baseline/candidate 재사용 확인 |
| 로컬·CI 실행 결과와 소요 시간 | fixed/latest 각각 17 passed, controls 30 passed; 로컬·CI PASS. revision·시간은 E-15 |
| PR / dev Draft | PR 0 #394·PR 1 #395·PR 2 #396 개인 통합 병합 완료; PR 3A/3B·dev Draft 미생성 |
...
|---|---|---|---|
| PR 3A | Decision | 복수 실제 DB connection의 경합·commit/rollback → 원본 응답·결과 artifact | 계획 확정·미구현 |
| PR 3B | Dashboard | 원본 artifact → 실제 client → 공유 변환 → 실제 launch의 다음 operation 인자 | 계획 확정·미구현 |
| PR 3 milestone | 두 저장소의 증거 연결 | Decision/Dashboard/Contract revision과 실제 소비 bundle hash가 연결된 결과 | 대장 준비·미실행 |

**개발자·AI의 시작점:** [계획 21절](implementation-plan.md#21-pr-3-milestone-실행-계약) → [새 검증 기준](verification-spec.md#pr-3-검증-경계--계획-아직-실행하지-않음) → [인수인계 절차](developer-guide.md#9-pr-3a에서-pr-3b로-넘기는-절차--구현-예정). 먼저 학습하려면 [7~9단원](learning-guide.md#7-pr-3a-동시-시작과-실제-경합은-다르다)을 읽는다. 완료 여부는 [E-16 대장](evidence-log.md#e-16-pr-3-milestone-결정과-증거-대장)으로 확인한다.
...
현재 PR 3 merge-safety verdict는 **incomplete evidence**다. 기존 로컬·CI 성공을 새 동시성·consumer 검증의 성공으로 확대하지 않는다. 각 저장소의 dev 제출·merge는 검증 상태와 별도로 관리한다.
