...
- 추천된 후보 묶음에서 각 후보가 기준선보다 유용한지 여부
...
빠르게 여러 기준일을 반복할 수 있다. 후보 타입, 조건 점수, 예상 전환율, 후보 선택은
별도의 백테스트 추천기가 아니라 현재 Decision 구현을 사용한다.

이 도구는 후보 선택과 추천 후보 묶음의 품질을 검증한다. `user_behavior_vectors` 배치 적재 자체와
확정 세그먼트의 광고 대상 배정은 기존 통합 테스트 및 운영 흐름의 검증 범위다.

프로모션 intent는 각 시나리오의 목적지와 시즌을 정답 JSON으로 고정한다. LLM의 표현
변동과 후보 선택 품질을 섞지 않기 위한 조치다. LLM intent 추출 정확도는 별도의 고정
...
4. 학습에 사용하지 않은 2014-01-01부터 2014-12-01까지의 후보별 예상값을 계산한다.
5. 2014년 미래 결과와 비교해 MAE, Brier score, 후보별 lift와 후보 묶음 유용성을 기록한다.

전체 후보 풀을 학습하는 이유는 보정 전 휴리스틱이 먼저 선택한 후보만 학습할 때 생기는
선택 편향을 막기 위해서다. 사용자에게 노출되는 개발 검증 결과에는 운영과 동일하게
사용자 중복 제한과 전략 다양성 기준을 적용한 최대 3개 후보만 기록한다.
...
4. 실제 목적지 일치 예약률, 전체 조건 일치자 대비 lift, 예약자 포착률, reach,
   표본 안정성, 후보 기준선 초과율을 비교해 축소 비율을 선택한다.
5. 선택한 비율을 2014년 검증 구간에 그대로 적용하고, 사전 기준을 통과한 경우에만
   런타임 정책으로 기록한다.

기본 판정 기준은 축소 정책이 적용된 후보 결과가 3개 이상이고, 전체 조건 일치자보다
성과가 낮지 않으며, 실제 예약자의 80% 이상을 유지하고, 추천 후보 기준선 초과율의
하락이 5%p 이내인 것이다. 이 값들은 숨은 상수가 아니라 CLI 옵션과 정책 JSON에 함께 기록된다.
...
.venv/bin/python scripts/backtest_expedia_segments.py validation \
  ...
  --audience-selection-max-portfolio-success-rate-drop 0.05
```
...
후보 간 우열 순서를 증명하는 근거는 아니다. 기존 내부 순서 적중률이 충분하지 않았기
때문에 제품은 순위를 주장하지 않고 서로 다른 전략의 유용한 후보 묶음을 제시한다.
...
- 전체 추천 후보 기준선 초과율 `60% 이상`
- 유용한 후보가 하나 이상인 시나리오 비율 `70% 이상`
- 모든 추천 후보가 기준선을 넘은 시나리오 비율 `50% 이상`
- 추천 후보 평균 lift `0%p 이상`
- 시나리오별 최저 성과 후보의 평균 lift `0%p 이상`
- 결과가 있는 추천 후보 `18개 이상`
- 후보가 2개 이상인 시나리오 `6개 이상`
- 후보가 3개인 시나리오 `6개 이상`
- 전체 후보 MAE `3.5%p 이하`
- 예측 편향 절댓값 `1.5%p 이하`
...
- `portfolio_candidate_beats_baseline_rate`: 추천 후보 전체에서 기준 예약률을 이긴 후보 비율
- `portfolio_scenario_any_candidate_beats_baseline_rate`: 유용한 후보가 하나 이상인 시나리오 비율
- `portfolio_scenario_all_candidates_beat_baseline_rate`: 모든 추천 후보가 기준선을 이긴 시나리오 비율
- `portfolio_mean_candidate_lift_percentage_points`: 추천 후보 전체의 평균 lift
- `portfolio_mean_worst_candidate_lift_percentage_points`: 각 시나리오의 최저 성과 후보를 모은 평균 lift
- `portfolio_three_candidate_scenario_count`: 서로 다른 후보 3개를 생성해 모두 평가한 시나리오 수

봉인 최종 테스트는 관측 outcome이나 다중 후보 시나리오가 부족하면 `FAIL` 대신
`INCONCLUSIVE`로 기록한다. 충분한 근거가 있을 때만 예상값 보정과 추천 후보 묶음의
유용성을 함께 사용해 `PASS` 또는 `FAIL`을 결정한다. 저장 순서별 지표는 개발자 진단을
위해 남지만 제품 주장이나 봉인 합격 기준에는 사용하지 않는다.

이 기준과 후보 타입별 학습 지원 계약은 `expedia.sealed-final-test.v4` manifest에 사전
등록된다. manifest에는 후보 타입별 학습 사례 수와 지원 목록이 함께 기록되며, 봉인 후
이 계약이 달라지면 최종 평가를 시작하지 않는다. 아직 결과를 열지 않은 이전 버전
manifest가 있다면 새 코드와 모델을 동결한 뒤 v4 manifest를 다시 생성해야 한다.
