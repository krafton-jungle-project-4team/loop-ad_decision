...
이번 변경에서 사용자군 계산은 Decision이 모두 끝냅니다. Dashboard는 벡터, ANN, cosine, raw event 조건을 구현할 필요가 없습니다.

Dashboard가 할 일은 아래 네 가지입니다.

```text
1. 세그먼트 추천 카드
   → Decision이 저장한 사용자 수를 그대로 표시

2. 후보 체크
   → 현재 체크한 조합의 최종 배정 예상 인원을 preview에서 찾아 표시

3. 세그먼트 확정
   → 기존 Dashboard direct INSERT만 사용하지 않고
     Decision /analyses를 호출해 final snapshot을 먼저 생성

4. 광고 소재·실험
   → 현재 카드의 analysis_id + generation_id + segment_id 하나만 전달
   → Decision이 해당 카드의 final snapshot 회원을 그대로 assignment
```

화면 변화는 작습니다.

- `대표 표본`을 `단독 대상 사용자`로 변경
- 체크된 카드에만 `현재 선택 기준 최종 배정 사용자` 한 줄 추가
- 확정된 카드에서는 final snapshot의 실제 배정 사용자 수 표시

API server에서는 세그먼트 확정 방식이 중요하게 바뀝니다. 현재 confirm SQL이 target segment와 vector를 직접 만드는 방식은 V2에 사용할 수 없습니다. V2 확정은 반드시 Decision `/analyses`를 먼저 호출해야 합니다.

한 문장으로 정리하면 다음과 같습니다.

> Dashboard는 사용자군을 다시 계산하지 말고, Decision이 추천 때 만든 source snapshot과 확정 때 만든 final snapshot을 화면과 실행 요청에 정확히 연결하면 됩니다.

## 대시보드 담당자가 해야 할 일

### 필수 작업

- [ ] suggestion 조회에 nullable `audience_snapshot_id` 추가
- [ ] 추천 analysis의 `output_json.audience_allocation_preview_context` 조회
- [ ] source 카드 숫자는 `metadata_json.display_copy.audience`에서 그대로 표시
- [ ] 현재 체크한 `selected_segment_ids`와 같은 preview 하나만 찾아 표시
- [ ] 기존 V2 confirm direct INSERT를 Decision `/analyses` 호출 방식으로 변경
- [ ] Decision이 만든 confirmation target에 `suggestion_id`, `confirmed_by`, `confirmed_at`만 보강
- [ ] V2 확정에 manual/ChatKit 세그먼트를 자동으로 합치지 않음
- [ ] 확정 세그먼트 조회에 `audience_snapshot_id`, `allocation_plan_id`, `audience_reservation_state` 추가
- [ ] 확정 카드의 실제 인원은 final snapshot의 `final_user_count` 사용
- [ ] 콘텐츠 생성은 confirmation `analysis_id` 사용
- [ ] 카드 run 요청은 현재 카드의 `analysis_id`, `generation_id`, `segment_ids: [현재 segment_id]` 사용
- [ ] assignment 응답의 `matching_mode=analysis_snapshot_reuse`를 정상 성공으로 처리
- [ ] nullable legacy row 회귀 테스트와 V2 전체 여정 테스트 추가

### 하지 말아야 할 일

- Dashboard에서 ANN/cosine/raw predicate 계산
- source snapshot 사용자를 Dashboard에서 다시 나누기
- promotion의 latest analysis/generation을 현재 카드 출처 대신 사용
- A 카드 실행 시 같은 확정 묶음의 B를 자동으로 run에 포함
- V2 AI 후보와 manual/ChatKit 후보를 같은 확정 요청에 혼합
- 기존 물리 삭제 SQL을 V2 target에 그대로 적용
- 신규 환경변수, 기능 스위치, 별도 Dashboard 요청 모드 추가
...
→ Decision PR #288
→ Dashboard 변경
→ dev 전체 여정 smoke test
```

## Dashboard 담당 Codex용 실행 지시문

아래 블록을 Dashboard 레포를 작업하는 Codex에 그대로 전달할 수 있습니다.

```md
Dashboard `origin/main`을 기준으로 Segment Audience V2 연동을 구현해줘.

전제:

- Data Contract PR #25가 머지된 것으로 간주한다.
- Decision PR #288이 머지된 것으로 간주한다.
- Decision과 Data Contract 레포는 수정하지 않는다.
- 신규 환경변수, 기능 스위치, Dashboard 요청 모드를 추가하지 않는다.
- legacy/manual/ChatKit 흐름은 유지한다.

목표:

Dashboard는 사용자군을 계산하지 않는다. Decision이 추천 단계에서 만든 source snapshot 수치와 확정 단계에서 만든 final snapshot을 읽고, 카드·확정·콘텐츠·카드별 run에 연결한다.

반드시 구현할 것:

1. 추천 카드 read model
   - `promotion_segment_suggestions.audience_snapshot_id`를 nullable로 노출한다.
   - `metadata_json.display_copy.audience`의 세 숫자를 그대로 사용한다.
   - `total_eligible_user_count`는 분석 가능 사용자다.
   - `matching_user_count`는 행동 조건 부합이다.
   - `selected_user_count`는 단독 대상 사용자다.
   - 문자열 `audience_summary`를 파싱해 숫자를 만들지 않는다.

2. Allocation preview
   - recommendation analysis의
     `output_json.audience_allocation_preview_context`를 읽는다.
   - 현재 체크된 segment ID를 canonical 정렬한다.
   - `allocation_previews`에서 `selected_segment_ids`가 정확히 같은 항목 하나만 찾는다.
   - 선택된 카드에만 `현재 선택 기준 최종 배정 사용자`를 표시한다.
   - 미선택 카드에는 allocation count를 표시하지 않는다.
   - revision/version이 현재 context와 다르면 preview를 사용하지 말고 refetch한다.
   - 모든 조합을 화면에 나열하지 않는다.

3. V2 confirm orchestration
   - 현재 `ConfirmDashboardPromotionSegmentSuggestions` direct INSERT만으로 확정하지 않는다.
   - 현재 recommendation analysis의 accepted V2 suggestion 1~3개만 선택한다.
   - 서로 다른 recommendation analysis를 섞지 않는다.
   - V2와 legacy/manual/ChatKit을 섞지 않는다.
   - Decision `POST /decision/v1/promotions/{promotion_id}/analyses`를 호출한다.
   - body에는 project_id, campaign_id, promotion_id, 전체 선택 segment_ids,
     operator_instruction을 전달한다.
   - Decision이 반환한 confirmation analysis_id와 target_segments를 보존한다.
   - Decision이 만든 target row에 suggestion_id, confirmed_by, confirmed_at만 보강한다.
   - audience_snapshot_id, allocation_plan_id, reservation state, vector,
     estimated_size를 Dashboard 값으로 덮어쓰지 않는다.
   - 선택된 suggestion만 confirmed로 바꾼다.
   - 동일 요청 재시도가 안전해야 한다.

4. 확정 target read model
   - target의 analysis_id, audience_snapshot_id, allocation_plan_id,
     audience_reservation_state를 nullable로 노출한다.
   - final snapshot을 join해 final_user_count, meets_min_sample_size,
     audience_status를 노출한다.
   - V2 확정 카드의 실제 사용자 수는 final snapshot 값을 사용한다.
   - legacy row의 snapshot 필드는 NULL이어도 정상 파싱한다.

5. 콘텐츠와 카드별 run
   - 콘텐츠 생성은 Decision confirm이 반환한 confirmation analysis_id를 사용한다.
   - 카드의 `광고 소재·실험` 요청은 다음 값을 반드시 전달한다.
     - analysis_id: 현재 카드 target의 confirmation analysis
     - generation_id: 현재 카드 승인 콘텐츠의 generation
     - segment_ids: `[현재 카드 segment_id]`
   - promotion의 latest analysis/generation으로 대체하지 않는다.
   - 같은 allocation plan의 다른 카드를 자동 포함하지 않는다.
   - assignment의 `matching_mode=analysis_snapshot_reuse`와 ANN 진단 0은 정상이다.

6. UI 문구
   - `대표 표본`을 `단독 대상 사용자`로 바꾼다.
   - 선택된 카드에 `현재 선택 기준 최종 배정 사용자`를 표시한다.
   - `insufficient_sample`은 실험 가능하지만 평가 표본 부족으로 안내한다.
   - `no_eligible_audience` 또는 preview `targetable=false`는 확정을 막는다.

7. 삭제 경계
   - 기존 물리 삭제 SQL을 V2 target에 적용하지 않는다.
   - V2 release API가 연결되기 전에는 V2 삭제 버튼을 비활성화한다.
   - legacy 삭제 흐름은 유지한다.

필수 테스트:

- legacy suggestion/target nullable 호환
- source 카드 세 숫자 표시
- A+B 선택 시 A/B final preview 표시, C allocation 미표시
- A+B+C로 선택 변경 시 세 카드 preview 동시 갱신
- stale revision preview 거부/refetch
- V2 confirm이 Decision `/analyses`를 호출
- manual/ChatKit 자동 합산 금지
- confirm 재시도 멱등성
- confirmation analysis로 콘텐츠 생성
- A 카드 run에 A만 포함되고 B는 포함되지 않음
- assignment가 snapshot reuse로 완료
- 구조화된 409/422 오류 메시지 처리

구현 전 현재 Dashboard 코드를 확인하고, shared schema → SQL/read model → provider/service → UI → contract/integration test 순서로 작업해줘. 상세 필드와 예시는 이 handoff 문서의 나머지 절을 기준으로 한다.
```
...
### 3.1 기존 카드 수치는 Decision 값을 그대로 사용
...
### 3.2 suggestion read model에 source snapshot ID 추가
...
### 3.3 audience schema의 additive 필드 보존
...
### 5.1 현재 direct confirm SQL을 V2에 그대로 사용하면 안 됨
