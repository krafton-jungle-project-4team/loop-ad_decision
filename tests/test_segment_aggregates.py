...
def test_normalize_dimensions_and_segment_key_are_stable() -> None:
    ...
    assert (
        ...
        == "region_서울__age_30s__gender_male__device_mobile_web__channel_unknown__category_unknown"
    )
...
def test_fetch_segment_aggregates_calculates_metrics_and_null_rates() -> None:
    ...
    assert aggregate.segment_key == "region_seoul__age_30s__gender_male__device_mobile_web__channel_kakao__category_fresh_food"
    ...
    assert "JSONExtractString(properties_json, 'region')" in client.queries[0]
